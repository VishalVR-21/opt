function make_submission() {
  ...
  if (document.getElemetn) var number_of_empty_files = 0;

  for (var i = 1; i <= 5; i++) {
    if (!document.getElementById("file" + i).files[0])
      number_of_empty_files += 1;
    else if (
      document.getElementById("file" + i).files[0].size / 1024 >
      2 * 1024
    ) {
...
function verify() {
  ...
  document.getElementById("my-submissions").href =
    "/my-submissions?passcode=" + passcode;
  ...
  if (data["ok"]) {
    ...
    for (var i = 0; i < 5; i++) {
      document.getElementById("best-points-" + (i + 1)).innerHTML =
        best_points[i];
